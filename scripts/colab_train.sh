...
echo "[2/6] ensure Colab session (A100→L4→T4)"
colab sessions | grep -q "$SESSION" || {
  colab new -s "$SESSION" --gpu A100 || colab new -s "$SESSION" --gpu L4 || colab new -s "$SESSION" --gpu T4
}
sleep 5
...
asset = "https://github.com/mikewong23571/x-ocr/releases/download/data-v1.0-textspans/x-ocr-bundle.zip"
vm = "$VM"
