...
class ScreenParser:
    ...
    def __call__(self, image) -> list[dict]:
        ...
        out = self.sess.run(None, {self.iname: x})[0]  # (1, N, 5+nc) or (N, 6)
        pred = out[0] if out.ndim == 3 else out
        # ultralytics export layout: [x1,y1,x2,y2,conf,cls...]; keep max class conf
        boxes = pred[:, :4] / scale
        confs = pred[:, 4]
        cls = pred[:, 5:].argmax(1) if pred.shape[1] > 6 else pred[:, 5].astype(int)
