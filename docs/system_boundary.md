...
## 8. 数据中转架构（2026-09-24 更新）— ✅ 闭环

问题：Colab VM 是临时的（/content 每次清空），若每次训练都从笔记本 `colab upload` 数据集，
Stage 1（1.5–4GB）/Stage 2（6–15GB）会产生大量重复上行流量，不合理。

方案（已实测）：
- **代码**：公开仓库 https://github.com/mikewong23571/x-ocr （本地 main → 每次重建单 commit 的
  public-main 分支 force-push，保证公开历史不含个人数据；`data/`、`.env.*` 均不入库）。
- **数据集**：GitHub Release 资产（[data-v0.1](https://github.com/mikewong23571/x-ocr/releases/tag/data-v0.1)，
  19.8MB）。`scripts/colab_train.sh` 现在在 VM 内直接从 Release URL 下载（urllib），
  **笔记本零数据流量**；失败时回退 `colab upload`。
- **实测**：Colab CPU VM 从 Release 拉取 19.8MB 耗时 **1.0s**（Google↔GitHub 骨干网）。
- **规模上限**：单 Release 资产 2GiB。Stage 1/2 数据集将分卷 zip（`zip -s 1000m`）多资产发布。

替代方案（记录，未采用）：
- HuggingFace Datasets：对多 GB 公开 ML 数据最优（resumable 上传/下载），但需要用户的 HF token。
- Google Drive（colab CLI token 已含 drive.file scope）：私有性好，但 `colab drivemount` 需要人工交互，
  自动化需自写 Drive API 上传脚本。
- 单 VM 常活（不 stop）：单个工作时段内免重传，但跨 VM 生命周期仍需中转。

隐私注记：Release 资产含真实 X 截图（用户本人 timeline）。当前按用户指示走公开仓库；
如需收紧，改 private repo + VM 内 gh token 下载即可，脚本只需换 URL。
