...
| 8 | 训练完成的小模型 checkpoint | experiments/exp002_baseline_nano/ | 完成(baseline) | yolo11n 2.6M：60ep@1280，**mAP50 0.542 / mAP50-95 0.466**（87 图、test=7 图噪声大）；exp001_smoke 亦有 |
| 9 | ONNX 导出 | experiments/exp002_baseline_nano/best_onnx_*.onnx | 完成 | fp32/fp16 已下载并**本地实测**（CPU ONNX Runtime 单帧 53 检测）；INT8 未做 |
...
| 12 | error analysis visualization | [evaluation/error_analysis.py](../evaluation/error_analysis.py) | 完成(待更大数据重跑) | exp002 FP/FN/低置信 montage + summary.json 已产出并回下载 |
...
| §四 | 仅 X Desktop Web；3 viewport；明暗双主题 | 3 viewport ✓；**双主题受阻**：扩展 Input 通道不可用（实证 0 事件到达），X 主题色块无法合成点击 → light 主题待用户手工切换一次（Display→Default）后即可采 |
...
| §九 | Stage 0 500–1000 张 → 人工 overlay 检查 | **进行中**：战役后 224 样本/14293 元素/81 组（s01–s08），30 类无零样本；data-v0.3 已发布；两轮 VLM overlay 抽查通过（对齐良好，仅小瑕疵）；下一批扩展至 500–1000（含 light 主题，等用户切换） |
