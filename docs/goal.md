...
| §十 | coverage matrix（7 route × ~17 状态） | **完成**：7 route×scroll×3-4viewport×6 弹窗态 + **未登录批次（独立浏览器 session）：登录墙/empty/skeleton/error 态 36 张**（taxonomy 30→32：login_input/login_button）；loading 骨架为 600ms 早期帧口径 |
...
## 修复记录（2026-09-24：用户报告「post 正文和用户名混淆」）

**根因（两个，模型本身无辜）**
1. `runtime/screen_doc.py` 分组缺陷：一个 post_container 检测框可横跨多条帖子 → 池内所有元素并成一组，author 取最后一个 username、text 取第一个 post_text → 张冠李戴（像素级 OCR 复现：Saber 的正文配上地坛公园的用户名；模型 4/4 检测框全部正确）。
2. 采集/校准竞态：DOM 提取与截图是两条命令，X 懒加载重排（新帖 pill 收起、compose 框位移）使 DOM 坐标过期 ~110px —— 训练标注与运行时 DOM 校准都可能对错位。

**修复**
- screen_doc 改为 username 锚定分组：列左对齐 username 开新帖组，正文/操作行归上方最近 username；引用推内缩进 username 只标注 quoted_author；视口截断帖保留为匿名帖（author=null）；无 username 时回退 container 分组。复测：diag3 两帖配对全对；home_scrolled/bookmarks/grok/explore 回归通过。
- `collector/collect.py` capture()：截图先行 + 双重 DOM 提取布局指纹比对，不稳定帧丢弃（防训练标注污染）；`runtime/hybrid.py` extract_dom(stable=True) 同守卫，重排期跳过校准回落纯视觉。


## 实战采集 + 识图审计记录（2026-09-24，session=audit0924）

- 实采 8 帧（home×4 + explore×4，70s），稳定性守卫下全部 stable=True；fuse phash 去重后 6 帧入库（48,155 元素全库）。
- VLM 逐帧识图审计 6 张 overlay：**零系统性偏移**（竞态修复生效）；VLM 报 4 处疑点，DOM 真值仲裁后 3 处误报、1 处设计使然（news_card 按定义框整卡）；真实缺口仅 taxonomy 已知的 views/体育卡行（非交互，M1 清单）。
- 模型×新帧 crosscheck：391 DOM 真值元素，IoU≥0.3 匹配 353（90%）；类别交换仅 2 起（post_media↔video 语义近邻 ×1、video→post_container ×1）；**username_link↔post_text 交换 0 起**——混淆根因确认为 screen_doc 分组（已修），非模型类别缺陷。
- 方法论沉淀：VLM 审计当疑点生成器、DOM 真值做仲裁、像素条带 OCR 做终审——与 flywheel 同构的第三信号。
