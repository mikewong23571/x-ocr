...
| §九 | Stage 0 500–1000 张 → 人工 overlay 检查 | **完成（数量门）**：18 个采集 session（s01–s18）→ **511 样本 / 32,619 元素**（450/40/21 冻结切分）；data-v0.3→v0.7 发布；两轮 VLM overlay 抽查通过；**light 主题批次仍待用户手工切换**（唯一覆盖缺口） |
