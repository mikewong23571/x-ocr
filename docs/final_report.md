...
> 目标源：[task_spec.md](task_spec.md) · 验收台账：[goal.md](goal.md) · **v1.3 终版**（+未登录批次：登录墙/empty/skeleton/error，taxonomy 32 类，exp009）
...
| 问题 | 答案 |
|---|---|
| 最终用了多少 screenshot？ | **681 张**（23 个采集 session：22 个登录态（dark 511 + light 134）+ **1 个未登录独立浏览器 session 36 张**（登录墙/empty/skeleton/error））|
| 多少 UI element instance？ | **41,190 个标注实例**（**32 类**，无零样本类；新增 login_input/login_button 覆盖未登录态）|
...
| 切分？ | 227 个 session×route 组防泄漏 split：train 584（含全部 light）/ val 40 / test 21（test 自 v0.3 起冻结=纯暗色，所有模型同 test 可比）；hard_test 120 样本独立目录 |
...
| 关键指标（frozen test 21 图） | 数据规模爬坡（同冻结 test 21 图）：87 图 mAP50 .542* → 224 图 **.845**（KIER .813）→ +11 定向 .879 → **511 图 nano .933 / mAP50-95 .860**、**small .947–.954 / .908–.913**（两次独立训练复现）；small@224 KIER .880（终版 KIER 见 eval_frozen，VM 回收未及回传——同构口径推算 >.90）|
...
| **511（exp006/007 终版）** | **nano .933 / small .947** | 数量+多样性共同驱动 |
| 645（exp008，+134 light，A100） | nano **.932/.872** | 暗色无损 + light 召回 2.4%→91.9% |
| 681（exp009，+36 未登录，32 类，A100） | nano **.930/.866，R .899** | 类数+2 基本无损；登录墙 probe：4 屏检出 21 个 login_button(.97+)/7 个 login_input(.90+) |
| 500–1000 | 待采（light 主题 + 稀有类定向） | — |

收益递减判断：87→224（+137）+0.303；224→235（+11 定向）+0.034；235→511（+276 混合）nano +0.054（.879→.933）——粗放采集明显递减，多样性（新 session/route/状态）仍是主驱动。**不需要 50K**：小模型在单一应用域的容量有限，预计 1–3K 高多样性样本即近饱和；把预算投给多样性（主题/route/状态）而非数量。下一版最值得加：light 主题全量、dialog/link_card/notification 的足量实例（各 >200）、独立浏览器会话采集。
...
- **双主题已闭环**：light 采集曾受阻（扩展 Input 通道 0 事件 + 合成事件被 X 拒绝 + 主题为账号级服务端设置），最终由用户经 RustDesk 远程手工切换 Default 后补齐 134 张。**零样本 light 召回 2.4% → 训练后 91.9%**（同 40 张 light probe，in-train 口径——held-out light 测试需 light test 组，留 v2）；暗色冻结 test 无损（exp008 .932/.872）
