...
> 目标源：[task_spec.md](task_spec.md) · 验收台账：[goal.md](goal.md) · **v1.1 终版**（Stage 0 511 张 + exp006/007 终版数字）
...
| 问题 | 答案 |
|---|---|
| 最终用了多少 screenshot？ | **511 张**（18 个采集 session s01–s18：4 种 viewport × 7 route × 菜单/弹窗/线程/通知状态，dark/lightsout 主题）|
| 多少 UI element instance？ | **32,619 个标注实例**（30 类，无零样本类）|
...
| 切分？ | 165 个 session×route 组防泄漏 split：train 450 / val 40 / test 21（test 自 v0.3 起冻结，所有模型同 test 可比）；hard_test 100 样本独立目录 |
...
- **仅 dark/lightsout 主题**：light 采集被环境阻断（扩展 Input 通道不可用——CDP 鼠标/键盘事件 0 到达，已实证；X 主题色块对合成 click/KeyboardEvent 均不响应；主题为账号级服务端设置无本地存储可改）——当前模型对 light 主题泛化**未验证**。解除方式：用户在 X More→Display 切到 Default 后运行一轮 light 采集即可补齐（约 30 分钟）
