...
def main() -> None:
    ...
    ap.add_argument("--freeze-from", default=None,
                    help="previous export_meta.json: keep its test/val groups frozen; new groups → train")
    args = ap.parse_args()
    ...
    if args.freeze_from:
        prev = json.loads((ROOT / args.freeze_from).read_text())
        prev_test = set(prev.get("groups_test", []))
        prev_val = set(prev.get("groups_val", []))
        test_g = prev_test & set(groups)
        val_g = prev_val & set(groups)
        train_g = set(groups) - test_g - val_g  # new groups all → train
        print(f"frozen split from {args.freeze_from}: test={len(test_g)} val={len(val_g)} train={len(train_g)} (+{len(set(groups) - prev_test - prev_val)} new)")
    else:
    ...
    data_yaml = {
        "path": ".",  # relative to this yaml; train_yolo.py rewrites to absolute at train time
        "train": "images/train",
        "val": "images/val",
        "test": "images/test",
